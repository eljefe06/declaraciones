...
import { declaracionesMetadata, stats, adminDeleteDeclaracion } from '@api/declaracion';
...
export class DeclaracionesComponent implements AfterViewInit, OnInit {
  ...
  getDeclaranteName(owner: any): string {
    if (!owner) return '';
    const parts = [owner.nombre, owner.primerApellido, owner.segundoApellido].filter((p: string) => !!p);
    const full = parts.join(' ').replace(/\s+/g, ' ').trim();
    return full !== '' ? full : owner.username || '';
  }
  ...
  confirmAdminDeleteDeclaration(row: any) {
    const nombre = this.getDeclaranteName(row.owner);
    const tipo = row.tipoDeclaracion;
    const dialogRef = this.dialog.open(DialogComponent, {
      data: {
        title: '¿Eliminar declaración?',
        message: `Vas a eliminar la declaración ${tipo} de ${nombre}. Esta acción se puede revertir desde la base de datos, pero la declaración desaparecerá del panel.`,
        trueText: 'Eliminar',
        falseText: 'Cancelar',
      },
    });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        this.adminDeleteDeclaration(row._id);
      }
    });
  }

  async adminDeleteDeclaration(id: string) {
    try {
      await this.apollo
        .mutate({
          mutation: adminDeleteDeclaracion,
          variables: { id },
        })
        .toPromise();
      this.presentAlert('Declaración eliminada', 'La declaración ha sido marcada como eliminada.');
    } catch (error) {
      console.log(error);
