...
export default {
  Query: {
    ...
    lastDeclaracion(_: unknown, args: unknown, context: Context): Promise<Declaracion | null> {
  ...
  Mutation: {
    ...
    adminDeleteDeclaracion(_: unknown, args: { id: string }, context: Context): Promise<boolean> {
      return DeclaracionRepository.adminSoftDelete(args.id, context.user.id);
    },
